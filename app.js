...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const masterExerciseList = {
        "Chest": [
            { name: "Barbell Bench Press", type: "free" }, { name: "Dumbbell Bench Press", type: "free" },
            { name: "Incline Dumbbell Press", type: "free" }, { name: "Machine Chest Press", type: "machine" },
            { name: "Incline Machine Press", type: "machine" }, { name: "Dumbbell Flyes", type: "free" },
            { name: "Pec Deck Machine", type: "machine" }, { name: "Cable Crossover", type: "machine" },
            { name: "Weighted Dips", type: "free" }, { name: "Push-ups", type: "free" }
        ],
        "Back": [
            { name: "Deadlift (Conventional)", type: "free" }, { name: "Barbell Row", type: "free" },
            { name: "Dumbbell Row", type: "free" }, { name: "Pull-ups", type: "free" },
            { name: "Chin-ups", type: "free" }, { name: "Lat Pulldown", type: "machine" },
            { name: "Seated Cable Row", type: "machine" }, { name: "T-Bar Row", type: "free" },
            { name: "Machine Pullover", type: "machine" }
        ],
        "Shoulders": [
            { name: "Overhead Press (Barbell)", type: "free" }, { name: "Machine Shoulder Press", type: "machine" },
            { name: "Arnold Press", type: "free" }, { name: "Dumbbell Lateral Raise", type: "free" },
            { name: "Cable Lateral Raise", type: "machine" }, { name: "Face Pulls", type: "machine" },
            { name: "Barbell Shrugs", type: "free" }
        ],
        "Quads": [
            { name: "Barbell Back Squat", type: "free" }, { name: "Front Squat", type: "free" },
            { name: "Leg Press Machine", type: "machine" }, { name: "Hack Squat Machine", type: "machine" },
            { name: "Bulgarian Split Squat", type: "free" }, { name: "Goblet Squat", type: "free" },
            { name: "Leg Extension Machine", type: "machine" }
        ],
        "Hamstrings & Glutes": [
            { name: "Romanian Deadlift", type: "free" }, { name: "Hip Thrust (Barbell)", type: "free" },
            { name: "Good Mornings", type: "free" }, { name: "Lying Leg Curl", type: "machine" },
            { name: "Seated Leg Curl", type: "machine" }, { name: "Hip Extension (45-degree)", type: "free" },
            { name: "Hip Adductor Machine", type: "machine" }, { name: "Hip Abductor Machine", type: "machine" }
        ],
        "Arms": [
            { name: "Dumbbell Bicep Curls", type: "free" }, { name: "Hammer Curls", type: "free" },
            { name: "Bicep Curl Machine", type: "machine" }, { name: "Close Grip Bench Press", type: "free" },
            { name: "Skull Crushers (EZ-Bar)", type: "free" }, { name: "Overhead Tricep Extension (Dumbbell)", type: "free" },
            { name: "Tricep Pushdown (Rope)", type: "machine" }, { name: "Tricep Pushdown (Bar)", type: "machine" }
        ],
        "Calves": [
            { name: "Standing Calf Raise", type: "machine" }, { name: "Seated Calf Raise", type: "machine" },
            { name: "Leg Press Calf Raise", type: "machine" }
        ],
        "Abs": [
            { name: "Crunches", type: "free" }, { name: "Leg Raises (Hanging or Lying)", type: "free" },
            { name: "Plank", type: "free" }, { name: "Cable Crunches", type: "machine" },
            { name: "Russian Twists", type: "free" }, { name: "Ab Wheel Rollouts", type: "free" }
        ]
    };
    
    function getExerciseDetails(exerciseName) {
        for (const category in masterExerciseList) {
            const foundExercise = masterExerciseList[category].find(ex => ex.name === exerciseName);
            if (foundExercise) return foundExercise;
        }
        return { name: exerciseName, type: 'free' };
    }
    ...
    function buildHeader() {
        ...
        getEl('history-btn').addEventListener('click', () => { renderHistoryList(); showView('history-list-view'); });
        getEl('home-btn').addEventListener('click', () => { renderRoutineList(); showView('routine-list-view'); });
        getEl('dark-mode-btn').addEventListener('click', () => { applyDarkMode(!document.body.classList.contains('dark-mode')); });
    ...
    function toggleUnit() {
        ...
        if (getEl('workout-player-view').style.display === 'block' && !currentWorkout.isResting) {
            const weightInput = getEl('workout-player-view').querySelector('.log-weight-input');
            if (weightInput) {
                const currentWeight = parseFloat(weightInput.value);
                if (!isNaN(currentWeight) && currentWeight > 0) {
                    convertedWeight = weightUnit === 'lbs' ? currentWeight / 2.20462 : currentWeight * 2.20462;
                }
            }
        }
        ...
        if (getEl('workout-player-view').style.display === 'block' && !currentWorkout.isResting) {
            updatePlayerUI(convertedWeight);
        }
    }
    ...
    // --- Routine List & Editor (MAJOR CHANGES HERE) ---
    ...
    function prepareRoutineEditor(routineId = null) {
        ...
        // ** MODIFIED: Build library AFTER populating routine list to get initial state **
        ...
        if (routine && routine.exercises.length > 0) {
            ...
            routine.exercises.forEach(ex => {
                const item = createRoutineExerciseItem(ex.name);
        ...
        buildLibrary(); // Now build library, which will check the routine list
        ...
        new Sortable(routineListEl, { group: 'shared', animation: 150, onAdd: handleDragAdd });
    ...
    // ** NEW: Central function to update a library item's state **
    function updateLibraryItemState(exerciseName, isAdded) {
        queryAll('.library-item').forEach(item => {
            if (item.textContent === exerciseName) {
    ...
    function handleLibraryItemClick(e) {
        ...
        if (item.classList.contains('is-added')) return; // Prevent adding duplicates

        const exerciseName = item.textContent;
        const routineListEl = getEl('routine-exercise-list');
        const newItem = createRoutineExerciseItem(exerciseName);
        routineListEl.appendChild(newItem);
        updateLibraryItemState(exerciseName, true); // Deactivate in library
        
        const placeholder = routineListEl.querySelector('.drop-zone-placeholder');
        if (placeholder) placeholder.style.display = 'none';
    }

    // ** NEW: Handler for when item is added via drag-and-drop **
    function handleDragAdd(evt) {
        const oldItem = evt.item;
        const exerciseName = oldItem.textContent;
        
        // Check if it's already in the list
        let isDuplicate = false;
        evt.to.querySelectorAll('.routine-exercise-item').forEach(item => {
            if (item !== oldItem && item.textContent.includes(exerciseName)) {
                isDuplicate = true;
            }
        });

        if (isDuplicate) {
            oldItem.remove(); // Remove the dragged duplicate
            return;
        }

        const newItem = createRoutineExerciseItem(exerciseName);
        evt.to.insertBefore(newItem, oldItem);
        evt.to.removeChild(oldItem);
        updateLibraryItemState(exerciseName, true); // Deactivate in library

        const placeholder = evt.to.querySelector('.drop-zone-placeholder');
        if (placeholder) placeholder.style.display = 'none';
    }

    function buildLibrary() {
        const container = getEl('library-container');
        container.innerHTML = '';
        // ** NEW: Get a set of exercises already in the routine for quick lookup **
        const existingExercises = new Set(
            Array.from(getEl('routine-exercise-list').querySelectorAll('.routine-exercise-item'))
            .map(item => item.childNodes[0].nodeValue.trim())
        );
        ...
        for (const category in masterExerciseList) {
            ...
            masterExerciseList[category].forEach(ex => {
                const item = document.createElement('div');
                item.className = 'library-item';
                item.textContent = ex.name;
                // ** NEW: Deactivate if already present **
                if (existingExercises.has(ex.name)) {
    ...
    // ** MODIFIED: createRoutineExerciseItem now handles re-activating library items on removal **
    function createRoutineExerciseItem(name) {
        const item = document.createElement('div');
        ...
        removeBtn.addEventListener('click', () => {
            item.remove();
            updateLibraryItemState(name, false); // Re-activate in library
        });
    ...
    // Player Logic and Dev Mode functions (no changes needed)
